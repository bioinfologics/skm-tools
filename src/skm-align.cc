...
int main(int argc, char * argv[]) {
    ...
    try
    {
        cxxopts::Options options("skm-align", "Simple aligner based on Skipmer direct matches");

        options.add_options()
                ("help", "Print help")
                ("t,target", "target sequence set", cxxopts::value<std::string>(target_filename))
                ("gff3_file", "gff3 file for features on the target sequence set", cxxopts::value<std::string>(gff3_filename))
                ...
                ("q,query", "coverage set (use as many as needed)", cxxopts::value<std::string>(query_filename))
    ...
    if (!gff3_filename.empty()) {
        target.mark_with_gff3_feature(gff3_filename, gff3_feature, minfeature);
        std::cout << "the feature collection has " << target.reffeatures.size() << " features" << std::endl;
    }
    ...
    ini = std::chrono::system_clock::now();
    query.create_from_reference_file(query_filename,true);
    uint64_t query_last=query.skipmer_positions.back().position;
    std::cout<< query.skipmer_positions.size() << " skip-mer positions, last position "<<query_last<<"."<<std::endl;
    ...
    for (uint64_t ti=0,qi=0;ti<target.skipmer_positions.size() and qi<query.skipmer_positions.size();++ti){
        //std::cout<<"ti: "<<ti<<"  qi: "<<qi<<std::endl;
        while (query.skipmer_positions[qi].skipmer<target.skipmer_positions[ti].skipmer and qi<query.skipmer_positions.size()) ++qi;
        ...
        if (query.skipmer_positions[qi].skipmer==target.skipmer_positions[ti].skipmer){
            ...
            uint64_t ft=0;
            for (auto te=ti;te<target.skipmer_positions.size() and target.skipmer_positions[te].skipmer==target.skipmer_positions[ti].skipmer;++te) ++ft;
            //std::cout<<"ft: "<<ft<<std::endl;
            uint64_t fq=0;
            for (auto qe=qi;qe<query.skipmer_positions.size() and query.skipmer_positions[qe].skipmer==query.skipmer_positions[qi].skipmer;++qe) ++fq;
                ...
                for (auto tii=ti;tii<ti+ft;++tii) {
                    for (auto qii = qi; qii < query.skipmer_positions.size() and
                        ...
                        if (query.skipmer_positions[qii].rev == target.skipmer_positions[tii].rev)
                            ...
                            revmatches.emplace_back(query_last - query.skipmer_positions[qii].position,target.skipmer_positions[tii].position);
                    }
                }
            }
            ti=ti+ft-1;
