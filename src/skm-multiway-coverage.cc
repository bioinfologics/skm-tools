...
int main(int argc, char * argv[]) {
    ...
    for (auto ref_id=0;ref_id<refcount;++ref_id) {
        ...
        fin = std::chrono::system_clock::now();
        elapsed_seconds = fin-ini;
        std::cout << "Counting " << seq_filenames[ref_id] << " elapsed time: " << elapsed_seconds.count() << "s\n";
        ...
        fin = std::chrono::system_clock::now();
        elapsed_seconds = fin-ini;
        std::cout << "add_coverage_track time: " << elapsed_seconds.count() << "s ";
        ini = std::chrono::system_clock::now();
        auto cs=mwca.covered_by_all_stats();
        fin = std::chrono::system_clock::now();
        elapsed_seconds = fin-ini;
        std::cout << "covered_by_all_stats elapsed time: " << elapsed_seconds.count() << "s ";
        ...
        fin = std::chrono::system_clock::now();
        elapsed_seconds = fin-ini;
        std::cout << "covered_by_all_projected_stats elapsed time: " << elapsed_seconds.count() << "s\n";
        for (auto covx:cs) prog_cov<<","<<covx;
        prog_cov<<","<<cs[0]-cs[3]<<","<<cs[1]-cs[4]<<","<<cs[2]-cs[5];
        prog_cov<<std::endl;
